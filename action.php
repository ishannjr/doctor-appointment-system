...
if(isset($_POST["action"]))
{
	...
	if($_POST['action'] == 'patient_register')
	{
		...
		else
		{
			...
			$mail->Port = '25';
			$mail->SMTPAuth = true;
			$mail->Username = 'ishaan.mandliya@somaiya.edu';
			$mail->Password = 'ciobazvqozllezbj';
			$mail->SMTPSecure = '';
			$mail->From = 'ishaan.mandliya@somaiya.edu';
			$mail->FromName = 'Ishaan';
			$mail->AddAddress($_POST["ishaan.mandliya@somaiya.edu"]);
			$mail->WordWrap = 50;
			$mail->IsHTML(true);
	...
	if($_POST['action'] == 'patient_login')
	{
		...
		if($object->row_count() > 0)
		{
			...
			{
				if($row["email_verify"] == 'Yes' or $row["email_verify"] == 'No')
				{
